...
class NodeIntermediateRepresentation:
    def __init__(self, prefix, main, suffix):
        self.prefix = copy.copy(
            prefix
        )  # Prefix: how to get from parent node to start node.
        self.main = copy.copy(
            main
        )  # Output from the schema graph. How to get from start node to end node
    ...
    def compose(self, to_push):
        return NodeIntermediateRepresentation(
            [to_push, Call()] + self.prefix, self.main, self.suffix + [Return()]
        )

    def strip(self):
        return NodeIntermediateRepresentation(
            self.prefix[1:], self.main, self.suffix[:-1]
        )
