...
def hide_key_in_intermediate_representation(representation: list[RepresentationStep], key: Domain):
    steps = []
    for step in representation:
        if step.name == "STT":
            ...
            if idx >= 0:
                ...
                steps += [StartTraversal(start_columns), Expand(start_node, end_node, indices, [key]), EndTraversal(start_columns, end_columns)]
        steps += [step]
    return steps