...
def carry_keys_through_representation(representation: list[RepresentationStep], keys: list[Domain]):
    result = []
    for command in representation:
        if isinstance(command, StartTraversal):
            ...
            result += [StartTraversal(command.start_columns + keys), Project(start_node, end_node, indices)]
